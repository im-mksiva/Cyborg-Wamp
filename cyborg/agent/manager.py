...
from cyborg.agent.wamp import WampAgent, start_wamp
from cyborg.agent import wamp
...
class AgentManager(periodic_task.PeriodicTasks):
    ...
    def __init__(self, topic, host=None):
        ...
        # self.wamp = WampAgent()
        wamp_thread = Thread(None, start_wamp)
        wamp_thread.start()
        # LOG.info(SESSION)
    ...
    def custom_method(self, context):
        LOG.info("funzione eseguita")

    def driver_creation(self, driver_name):
        driver = self.fpga_driver.create(driver_name)
        ret = driver.program()
        return ret
